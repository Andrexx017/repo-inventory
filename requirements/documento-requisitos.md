...
| DEP-03 | El backend depende de una cuenta de **Gmail con contraseña de aplicación** (`SMTP_USER`/`SMTP_PASSWORD`, vía MailKit contra `smtp.gmail.com:587`) para tres funcionalidades ya implementadas: recuperación de contraseña (`POST /api/auth/forgot-password`), notificación por correo de alertas de stock (RF-34, detrás del flag `Alerts:NotifyByEmail`) y envío manual de reportes por correo (RF-35, `POST /api/reports/{branchId}/send-now`). Sin esta cuenta configurada en `.env`, las tres funcionalidades fallan o quedan inactivas (la de alertas falla en silencio por diseño; el envío de reportes devuelve error al usuario; ver `backend/docs/decisions.md`). | Dependencia externa activa — pendiente de confirmar que el evaluador reciba credenciales SMTP válidas o que se documente cómo generarlas (cuenta Gmail + verificación en 2 pasos + contraseña de aplicación). |
